...
const VerticalMovieCard: React.FC<Props> = ({movie, onPosterPress}) => {
  ...
  useEffect(() => {
    async function fetchExtraDetails() {
      const movieDetails = await getQueued<TmdbMovie>(`movie/${movie.id}?append_to_response=credits`);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.movieDetails}>
        ...
        <View>
          <Text style={styles.actor} numberOfLines={1}>Actors: {actors}</Text>
          <Text style={styles.director} numberOfLines={1}>Directors: {directors}</Text>
          <View style={styles.votesGroup}>
            <View style={styles.starsAndDuration}>
              <Text style={styles.stars}>{stars}</Text>
              <Text style={styles.duration}>{runtime}</Text>
            </View>
            <Text style={styles.voteAverage}>{movie.vote_average.toFixed(1)}</Text>
          </View>          
        </View>
      </View>
    </View>
